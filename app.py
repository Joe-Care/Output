...
def get_app_post():
    ...
    c = con.cursor()
    try:
        sql_select_query = "select * from approvePosts where app = 'True';"
        c.execute(sql_select_query)
        record = c.fetchall()
    finally:
        con.commit()
...
def get_posts_profile(userid):
    ...
    c = con.cursor()
    try:
        sql_select_query = "select * from approvePosts where userid = ?;"
...
def anonymous():
    if request.method == 'POST':
        ...
        return render_template("post.html", info = "You post have been sent to moderation. It will be approved in 24 hours.", type = anony)
